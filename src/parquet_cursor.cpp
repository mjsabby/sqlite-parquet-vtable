...
bool ParquetCursor::currentRowGroupSatisfiesBlobFilter(Constraint &constraint, std::shared_ptr<parquet::Statistics> _stats)
{
    if (!_stats->HasMinMax())
    {
        return true;
    ...
    {
        parquet::ByteArrayStatistics *stats = (parquet::ByteArrayStatistics *)_stats.get();
    ...
    else
    {
        ...
        ss << __FILE__ << ":" << __LINE__
           << ": currentRowGroupSatisfiesBlobFilter called on unsupported type: " << parquet::TypeToString(pqType);
    ...
    case Equal: {
        ...
        bool blobGteMinBlob = std::lexicographical_compare(minPtr, minPtr + minLen, &blob[0], &blob[0] + blob.size());

        bool blobLtMaxBlob = std::lexicographical_compare(&blob[0], &blob[0] + blob.size(), maxPtr, maxPtr + maxLen);
    ...
    case GreaterThanOrEqual: {
        ...
        return maxEqual || std::lexicographical_compare(&blob[0], &blob[0] + blob.size(), maxPtr, maxPtr + maxLen);
    }
    case GreaterThan:
        return std::lexicographical_compare(&blob[0], &blob[0] + blob.size(), maxPtr, maxPtr + maxLen);
    case LessThan:
        return std::lexicographical_compare(minPtr, minPtr + minLen, &blob[0], &blob[0] + blob.size());
    case LessThanOrEqual: {
        bool minEqual = blob.size() == minLen && memcmp(&blob[0], minPtr, minLen) == 0;
        return minEqual || std::lexicographical_compare(minPtr, minPtr + minLen, &blob[0], &blob[0] + blob.size());
    }
    case NotEqual: {
...
bool ParquetCursor::currentRowGroupSatisfiesTextFilter(Constraint &constraint, std::shared_ptr<parquet::Statistics> _stats)
{
    parquet::ByteArrayStatistics *stats = (parquet::ByteArrayStatistics *)_stats.get();
...
{
    int64_t ns = rv.value[0] + ((unsigned long)rv.value[1] << 32);
    int64_t julianDay = rv.value[2];
    int64_t nsSinceEpoch = (julianDay - 2440588);
    nsSinceEpoch *= 86400;
...
bool ParquetCursor::currentRowGroupSatisfiesIntegerFilter(Constraint &constraint, std::shared_ptr<parquet::Statistics> _stats)
{
    if (!_stats->HasMinMax())
    {
        return true;
    ...
    {
        auto *stats = dynamic_cast<parquet::Int32Statistics *>(_stats.get());
    ...
    {
        auto *stats = dynamic_cast<parquet::Int64Statistics *>(_stats.get());
    ...
    {
        auto *stats = dynamic_cast<parquet::TypedStatistics<parquet::Int96Type> *>(_stats.get());
    ...
    {
        auto *stats = dynamic_cast<parquet::BoolStatistics *>(_stats.get());
    ...
    else
    {
        ...
        std::ostringstream ss;
        ss << __FILE__ << ":" << __LINE__
           << ": currentRowGroupSatisfiesIntegerFilter called on unsupported type: " << parquet::TypeToString(pqType);
        throw std::invalid_argument(ss.str());
    }
...
bool ParquetCursor::currentRowGroupSatisfiesDoubleFilter(Constraint &constraint, std::shared_ptr<parquet::Statistics> _stats)
{
    if (!_stats->HasMinMax())
    {
        return true;
    ...
    {
        auto *stats = dynamic_cast<parquet::DoubleStatistics *>(_stats.get());
    ...
    {
        auto *stats = dynamic_cast<parquet::FloatStatistics *>(_stats.get());
...
bool ParquetCursor::currentRowSatisfiesTextFilter(Constraint &constraint)
{
    ...
    case GreaterThan: {
        ...
        return std::lexicographical_compare(&blob[0], &blob[0] + blob.size(), ba->ptr, ba->ptr + ba->len);
    }
    case GreaterThanOrEqual: {
        const std::vector<unsigned char> &blob = constraint.blobValue;
        ...
        return equal || std::lexicographical_compare(&blob[0], &blob[0] + blob.size(), ba->ptr, ba->ptr + ba->len);
    }
    case LessThan: {
        const std::vector<unsigned char> &blob = constraint.blobValue;

        return std::lexicographical_compare(ba->ptr, ba->ptr + ba->len, &blob[0], &blob[0] + blob.size());
    }
    case LessThanOrEqual: {
        const std::vector<unsigned char> &blob = constraint.blobValue;
        ...
        return equal || std::lexicographical_compare(ba->ptr, ba->ptr + ba->len, &blob[0], &blob[0] + blob.size());
    }
    case Like: {
        const std::string &likeStringValue = constraint.likeStringValue;
...
bool ParquetCursor::currentRowSatisfiesIntegerFilter(Constraint &constraint)
{
    ...
    else
    {
        ...
        else
        {
            ...
            std::ostringstream ss;
            ss << __FILE__ << ":" << __LINE__
               << ": currentRowSatisfiesIntegerFilter called on unsupported type: " << parquet::TypeToString(pqType);
            throw std::invalid_argument(ss.str());
        }
    }
        ...
        else
        {
            ...
            if (md->is_stats_set())
            {
                ...
                else
                {
                    ...
                    if (pqType == parquet::Type::BYTE_ARRAY && isStringType(column))
                    {
                    ...
                    else if (pqType == parquet::Type::INT32 || pqType == parquet::Type::INT64 || pqType == parquet::Type::INT96 ||
...
start:
    ...
    rowGroup = this->table->getRowGroupReader(rowGroupId);
    const auto *const rowGroupMetadata = rowGroup->metadata();
...
bool ParquetCursor::currentRowSatisfiesFilter()
{
    ...
    for (unsigned int i = 0; i < constraints.size(); i++)
    {
        ...
        else
        {
            ...
            {
                parquet::Type::type pqType = types[column];
                if (pqType == parquet::Type::INT32 || pqType == parquet::Type::INT64 || pqType == parquet::Type::INT96 ||
                    pqType == parquet::Type::BOOLEAN)
                {
    ...
    if (colRows[col] != rowId)
    {
        ...
        while (colRows[col] + 1 < rowId)
        {
            ...
            default:
                ...
                // CREATE time -- maybe file changed underneath us?
                std::ostringstream ss;
                ss << __FILE__ << ":" << __LINE__ << ": column " << col << " has unsupported type: " << parquet::TypeToString(types[col]);
                throw std::invalid_argument(ss.str());
                break;
            }
            colRows[col]++;
...
bool ParquetCursor::isStringType(int col)
{
    return isColumnStringType[col];
}
...
void ParquetCursor::reset(std::vector<Constraint> constraints)
{
    ...
    numRowGroups = this->table->getNumRowGroups();
}

ParquetTable *ParquetCursor::getTable() const
{
    return table;
}

unsigned int ParquetCursor::getNumRowGroups() const
{
    return numRowGroups;
}
